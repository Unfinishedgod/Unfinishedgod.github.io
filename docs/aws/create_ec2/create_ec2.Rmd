...
탄력적 IP의 정보를 확인해주자. 특별한건 없고 바로 **탄력적 IP 연결**버튼 클ㄹ
...
그동안 aws ec2에서 rstudio server 구축 하는 블로그를 쓰겠다고 다짐만 한지 18개월이 지났다. 하려면 5분이면 충분히 할수 있는데 반해, 블로글르 끄기 위해서는 스크린샷을 23개나 찍어서 하나하나 설명 해야해서 나도 모르게 미뤄뒀다. 사실 그만큼 처음 하는사람에게는 진입장벽이 커서, 시도조차 모하는 사람이 있을텐데 잘 따라해서 성공적으로 aws생태계에 들어왔으면 하는 바램이다.
