...
p1 <- ggplot(data = kor_pop_corona) +
  ...
  labs(title = "시도별 인구(2020년 기준)", x = "지역(시도)", y = "", colors = "인구")
  
p2 <- ggplot(data = kor_pop_corona) +
  geom_col(aes(x = name, y = pop_corona, fill = pop_corona)) +
  theme(axis.text.x = element_text(size = 10, angle=30, hjust=1)) +
  labs(title = "시도별 코로나 확진 환자", x = "지역(시도)", y = "", colors = "인구")
...
kor_pop_corona %>% 
  mutate(
    ...
    `지역대비 확진자 비율` = paste0(format(round(pop_corona / pop2020 * 100, 4), scientific = FALSE),"%")) %>% 
  rename(총인구수 = pop2020, 확진자 = pop_corona, 행정구역 = name) %>% 
  select(시점,총인구수, 확진자, `지역대비 확진자`, `지역대비 확진자 비율`,행정구역) %>% 
  datatable(rownames = FALSE)
```
