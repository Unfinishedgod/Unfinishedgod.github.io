---
title: "House Prices: Advanced Regression Techniques(Doing)"
author: "최의용"
date: "`r Sys.Date()`"
output: 
  html_document:
...
>>>>>>> 7264d49b30bee39a6dbe46dfb75d922cf399e303
<br>
...
ggplot(data=all[!is.na(all$SalePrice),], aes(x=SalePrice)) +
        geom_histogram(fill="#00338D", binwidth = 10000) +
...
ggplot(data=all[!is.na(all$SalePrice),], aes(x=factor(OverallQual), y=SalePrice))+
        geom_boxplot(col='#00338D') + labs(x='Overall Quality') +
...
ggplot(data=all[!is.na(all$SalePrice),], aes(x=GrLivArea, y=SalePrice))+
        geom_point(col='#00338D') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
...
ggplot(all[!is.na(all$SalePrice),], aes(x=MiscFeature, y=SalePrice)) +
        geom_bar(stat='summary', fun.y = "median", fill='#00338D') +
        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
...
all <- all %>% 
  drop_na()
#  quick_RF <- randomForest(x=all[1:1460,-73], y=all$SalePrice[1:1460], ntree=100, importance=TRUE)
quick_RF <- randomForest(x=all[,-73], y=all$SalePrice, ntree=100, importance=TRUE)
...
#  layout <- matrix(c(1,2,5,4,8,6,7),4,2,byrow=TRUE)
grid.arrange(s1, s2,s6, s7, s8, nrow=3)
```
...
nb1 <- ggplot(all[!is.na(all$SalePrice),], aes(x=reorder(Neighborhood, SalePrice, FUN=median), y=SalePrice)) +
        geom_bar(stat='summary', fun.y = "median", fill='#00338D') + labs(x='Neighborhood', y='Median SalePrice') +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        scale_y_continuous(breaks= seq(0, 800000, by=50000), labels = comma) +
        geom_label(stat = "count", aes(label = ..count.., y = ..count..), size=3) +
        geom_hline(yintercept=163000, linetype="dashed", color = "red") # dashed line is median SalePrice
nb2 <- ggplot(all[!is.na(all$SalePrice),], aes(x=reorder(Neighborhood, SalePrice, FUN=mean), y=SalePrice)) +
        geom_bar(stat='summary', fun.y = "mean", fill='#00338D') + labs(x='Neighborhood', y="Mean SalePrice") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
        scale_y_continuous(breaks= seq(0, 800000, by=50000), labels = comma) +
        geom_label(stat = "count", aes(label = ..count.., y = ..count..), size=3) +
        geom_hline(yintercept=163000, linetype="dashed", color = "red") # dashed line is median SalePrice
...
```{r}
all$TotalPorchSF <- all$OpenPorchSF + all$EnclosedPorch + all$ScreenPorch #  + all$X3SsnPorch
```
...
```{r}
sub_avg <- data.frame(Id = test_labels, SalePrice = (predictions_XGB[-1]))
head(sub_avg)
